...
- **03 カーソルテーマ工房** (`/cursor-cape`)
  ...
  - cape ライブラリ `~/Library/Application Support/Mousecape/capes` の .cape をカードで一覧
    （**適用中バッジつき、カードの「適用」でテーマ切り替え**）、各カーソルをアニメーションプレビュー
  ...
  - 「適用」= `mousecloak --apply`、「標準カーソルに戻す」= `mousecloak --reset`（すべてローカル、外部送信なし）
